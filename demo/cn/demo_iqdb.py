...
def show_result(resp: IqdbResponse) -> None:
    ...
    logger.info(f"SauceNAO 搜图链接: {resp.saucenao_url}")
    logger.info(f"Ascii2d 搜图链接: {resp.ascii2d_url}")
    logger.info(f"TinEye 搜图链接: {resp.tineye_url}")
    logger.info(f"Google 搜图链接: {resp.google_url}")
