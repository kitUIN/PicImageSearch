...
♻ `ascii2d` 增加对来源为 `seiga` 或 `nijie` 的处理

🐛 修正 `ascii2d` 搜索结果的解析逻辑

## [3.7.0](https://github.com/kitUIN/PicImageSearch/releases/tag/v3.7.0)
♻ `saucenao` 返回结果增加 `ext_urls` `author_url` `source` ，去除 `pixiv_id` 和 `member_id`
...
♻ 重构 `ascii2d` 搜索结果的获取和解析逻辑，及返回的结果
...
♻ 重构 `baidu` 搜索结果的获取和解析逻辑，及返回的结果样式
...
♻ 重构 search 参数 `file` 为 `str` / `Path` 类型
